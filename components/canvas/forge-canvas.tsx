...
function IdeaNode({ data }: { data: { label: string; source: string; team: string; color?: string } }) {
  const teamColors: Record<string, string> = {
    rafael: "#F59E0B", aiona: "#F97316", gabriel: "#14B8A6",
    project: "#8B5CF6", default: "#3B82F6",
  };
  const color = data.color || teamColors[data.team as keyof typeof teamColors] || teamColors.default;
  return (
    <motion.div
      initial={{ scale: 0 }}
      animate={{ scale: 1 }}
...
function BubbleNode({ data }: { data: { label: string; team: string; childCount: number } }) {
  ...
  return (
    ...
    >
      ...
      {data.childCount > 0 && (
        <p className="text-xs text-[#64748B] mt-1">{data.childCount} items</p>
      )}
    </motion.div>
  );
}
...
function ArtifactNode({ data }: { data: { label: string; type: string; status: string } }) {
  const statusColors: Record<string, string> = {
    draft: "#EAB308", edited: "#3B82F6", published: "#22C55E",
  };
...
function ForgeCanvasInner({ initialNodes, initialEdges, onSave }: ForgeCanvasProps) {
  ...
  // Sync when project changes (via key prop forcing remount)
  useEffect(() => {
  ...
  const onConnect = useCallback(
    (params: Connection) => {
      setEdges((eds) => {
        const next = addEdge({ ...params, animated: true }, eds) as Edge[];
        triggerSave(nodes, next);
        return next;
      });
    },
      ...
      >
        ...
        <MiniMap
          nodeColor={(node) => {
            ...
            if (n.type === "bubble") return "#8B5CF6";
            if (n.type === "artifact") {
              const statusColors: Record<string, string> = { draft: "#EAB308", edited: "#3B82F6", published: "#22C55E" };
              return statusColors[n.data?.status as string] || "#64748B";
            }
      ...
      <AnimatePresence>
        {showAddMenu && (
          <motion.div
            ...
            className="absolute z-50 rounded-xl border border-[#1E293B] bg-[#0F172A] p-2 shadow-2xl"
            style={{ top: addMenuPos.y + 10, left: addMenuPos.x + 10 }}
          >
            <div className="flex flex-col gap-1">
              {[
                { type: "idea" as const, label: "Idea Node", icon: Lightbulb, color: "#F59E0B" },
                { type: "bubble" as const, label: "Bubble Node", icon: Puzzle, color: "#8B5CF6" },
...
export function ForgeCanvas(props: ForgeCanvasProps) {
  return (
